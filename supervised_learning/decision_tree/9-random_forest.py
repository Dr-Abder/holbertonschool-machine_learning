...
class Decision_Tree():
    ...
    def possible_thresholds(self, node, feature):
        """
        Récupère toutes les valeurs uniques de la feature choisie
        dans la sous-population du nœud.
        Calcule les milieux entre deux valeurs consécutives.
        """
    ...
    def Gini_split_criterion_one_feature(self, node, feature):
        """
        Fonctio qui permet de tester les coupures possibles d’une seule feature
        et de calculer le Gini:
        1 Récupère les données pour cette variable et ce nœud
        2 Teste tous les seuils possibles
        3 Pour chaque seuil, divise en "gauche" (≤ seuil) et "droite" (> seuil)
        4 Calcule les probabilités dans chaque groupe
        5 APPLIQUE LA FORMULE DE GINI : Gini = 1 - Σ(p_i²)
        6 Calcule le Gini pondéré total
        """
        ...
        comparison = sub_X[:, None] > thresholds[None, :]
        classes = np.unique(self.target)
        one_hot = sub_y[:, None] == classes[None, :]

        Left_F = comparison[:, :, None] & one_hot[:, None, :]

        Left_counts = Left_F.sum(axis=0)
        Right_counts = one_hot[:, None, :].sum(axis=0) - Left_counts

        Left_probs = Left_counts / Left_counts.sum(axis=1, keepdims=True)
        Right_probs = Right_counts / Right_counts.sum(axis=1, keepdims=True)

        Gini_left = 1 - (Left_probs ** 2).sum(axis=1)
        Gini_right = 1 - (Right_probs ** 2).sum(axis=1)
        ...
        best_gini = gini_avg[best_idx]

        return best_threshold, best_gini

    def Gini_split_criterion(self, node):
        """
        Applique la fonction précédente pour toutes les features.
        Compare les meilleurs seuils de chaque feature.
        """
...
class Random_Forest():
    ...
    def predict(self, explanatory):
        ...
        predictions_finales = result.mode
